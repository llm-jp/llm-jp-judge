...
from ..dataset.safety_regional import SafetyRegionalDatasetItem, SafetyRegionalDatasetItemForEvaluation
from .base import BaseEvaluator, BaseScoreExtractor


class SafetyRegionalEvaluator(BaseEvaluator):
    def __init__(self, *args, api_error_score: int | None = None, **kwargs):
        self.api_error_score = api_error_score
        super().__init__(*args, **kwargs)

    def __call__(
        self, responses: Sequence[SafetyRegionalDatasetItem]
    ) -> tuple[dict[str, float | None], dict[str, float]]:  # type: ignore[override]
        data: list[SafetyRegionalDatasetItemForEvaluation] = []
        for res in responses:
            ...
            d = SafetyRegionalDatasetItemForEvaluation(
                ID=res.ID,
                prompt=[prompt],
                text=res.text,
